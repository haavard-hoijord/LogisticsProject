...
public static class Planner
{
        ...
        if (vehicle != null)
            try
            {
                ...
                vehicle = await Program.client.InvokeMethodAsync<Vehicle>(
                    Program.client.CreateInvokeMethodRequest(HttpMethod.Get, "Database", "track", vehicle.id));
                vehicle.route = route;

                await Program.client.InvokeMethodAsync(
                    Program.client.CreateInvokeMethodRequest(HttpMethod.Post, "Database", "update", vehicle));
    ...
    private static async Task<Vehicle> FindFittingVehicle(Delivery data)
    {
        try
        {
            var message = Program.client.CreateInvokeMethodRequest(HttpMethod.Get, "Database", "track/all");
