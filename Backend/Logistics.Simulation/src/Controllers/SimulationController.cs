        ...
        try
        {
            ...
            foreach (var vehicle in obj)
                ...
                if (vehicle.route.sections.Count > 0)
                {
                    ...
                    await Program.client.InvokeMethodAsync(HttpMethod.Post, "Database", "update", vehicle);
                }
    ...
    public async void AddRandomVehicle([FromBody] RandomRequest request)
    {
        ...
        for (var i = 0; i < request.amount; i++)
        {
            ...
            try
            {
                ...
                var addressRequest = Program.client.CreateInvokeMethodRequest(HttpMethod.Post, "Deliveries", "address", data);
                var coordinate = await Program.client.InvokeMethodAsync<Coordinate>(addressRequest);

                var companies = await Program.client.InvokeMethodAsync<List<Company>>(Program.client.CreateInvokeMethodRequest(HttpMethod.Get, "Api", "companies"));
                ...
                var response = await Program.client.InvokeMethodWithResponseAsync(Program.client.CreateInvokeMethodRequest(HttpMethod.Post, "Database", "add", vehicle));
    ...
    public async void AddRandomDelivery([FromBody] RandomRequest request)
    {
        ...
        for (var i = 0; i < request.amount; i++)
        {
            try
            {
                ...
                var response = await Program.client.InvokeMethodWithResponseAsync(Program.client.CreateInvokeMethodRequest(HttpMethod.Post, "Deliveries", "add", delivery));
